...
exports.downloadWithForm = downloadWithForm;
exports.downloadCanvas = downloadCanvas;
exports.downloadHtmlAsImage = downloadHtmlAsImage;
...
var log = require('loglevel').getLogger('utils/download'); /**
                                                             * @module download
...
function downloadBlob(blob) {
    var filename = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'download';
    var extension = arguments[2];
    var mime = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';

    navigator.msSaveBlob(new Blob(Array.isArray(blob) ? blob : [blob], { type: TYPES[extension] || mime }), filename + '.' + extension);
}
 ...
 */
function downloadLink(url, params) {
    var paramsStr = '';
    if (_lodash2.default.isString(params)) {
        paramsStr = params;
    } else if (_lodash2.default.isObject(params)) {
        paramsStr = _queryString2.default.stringify(params);
    }
...
function downloadFile(content, filename, extension) {
    var options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : { blob: false, mime: '' };

    if ('download' in document.createElement('a')) {
        var enableBlob = _lodash2.default.get(options, 'blob', false);
        var mime = _lodash2.default.get(options, 'mime', '');

        if (enableBlob) {
            var blob = new Blob(content, { type: mime || TYPES[extension] });
            ...
            downloadDataUrl('data:' + (mime || TYPES[extension]) + ',' + encodeURIComponent(content), filename, extension);
        }

        return null;
    } else if (navigator.msSaveBlob) {
        // IE10+
        var _mime = _lodash2.default.get(options, 'mime', '');
        return downloadBlob(content, filename, extension, _mime);
    } else {
 ...
 */
function downloadWithForm(url, data) {
    // create form
 ...
 */
function downloadHtmlAsImage(dom, filename, extension) {
    var target = dom || document.body;
    (0, _html2canvas2.default)(target, { allowTaint: true, foreignObjectRendering: true }).then(function (canvas) {
