...
import set3 from '../mock/cibd.json';

const log = require('loglevel').getLogger('src/standard');
...
const DATA = {
  ...
  set3: _(set3)
    .map((el) => {
      if (el.type === 'marker') {
        const randCls = Math.floor(Math.random() * 4);
        const randGrp = Math.floor(Math.random() * 3);
        el.cluster = CLUSTER[randCls];
        el.group = GROUP[randGrp];
      }
...
Examples.GIS = class extends React.Component {
  ...
  componentDidMount() {
    const gis = new Gis('map', {
      /*
      // Example of using "esri" base-map provider
      baseMap: {
    ...
    // This off() removes the ctxHandler of polylines
  ...
  filterMarkers = () => {
    this.gis.filterSymbol(symbol => (symbol.type === 'marker'));
  ...
  showSymbols = (e) => {
    if (e.target.value) {
      this.gis.showSymbol(symbol => (symbol.type === e.target.value));
    } else {
      this.gis.showSymbol();
    }
  ...
  hideSymbols = (e) => {
    if (e.target.value) {
      this.gis.hideSymbol(symbol => (symbol.type === e.target.value));
    } else {
      this.gis.hideSymbol();
    }
      ...
      <div className="c-flex c-gis">
        ...
        <div className="c-flex fdc map-func">
          ...
          <div className="c-flex fdc">
            ...
            <button onClick={this.loadSymbols} value="set3">Load Symbols 3</button>
            ...
            <button onClick={this.selectGroup} value="G1">Select G1 Group</button>
...
export default class extends React.Component {
  render() {
    return (
      <div>
        {
          _.map(Examples, (example, key) => (React.createElement(createExample(example, key), { key })))
        }
      </div>
    );
  }
