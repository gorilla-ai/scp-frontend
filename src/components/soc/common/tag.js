...
class IncidentTag extends Component {
  ...
  render() {
    ...
    const actions = {
      ...
      confirm: {text: t('txt-save'), handler: this.confirm}
    };
...
class TagArray extends React.Component {
  ...
  render() {
    const {value: {id, tag, color, account}} = this.props;
    const options = _.map(RAINBOW, el => {
      return {
        value: el,
        label: <div className='incident-tag-square' style={{backgroundColor: el}} />
      };
    });
